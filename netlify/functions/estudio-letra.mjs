...
export default async (req) => {
  ...
  try {
    ...
    const director = await dirigirEscena(tema, { modalidad: 'discurso' });
    const guiaDirector = briefAGuiaOral(director);
...
Director Semántico VIAM (obedece significados y conjuntos, no solo palabras sueltas):
${guiaDirector || director.cuadro_completo}
Inferencias lógicas a respetar: ${(director.inferencias || []).join(' ') || 'ninguna extra'}
      ...
      const r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'Eres locutor y copywriter de video del Ecosistema VIAM. Solo devuelves discurso o texto hablado, nunca letra de canción. Obedeces el cuadro semántico completo.',
            },
        ...
        if (letra) {
          return Response.json({
            ...
            director: {
              ...
              inferencias: director.inferencias?.slice(0, 4) || [],
