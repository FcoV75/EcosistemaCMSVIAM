...
export default async (req) => {
  ...
  try {
    ...
    let memberData = await store.get(normalized, { type: 'json' });
    ...
    const estado = estadoMembresia(normalized, memberData || { producto: 'sincronia_nexus' });
    ...
    if (!esPermanente && memberData?.startDate != null) {
      ...
      await store.setJSON(normalized, memberData);
    }
