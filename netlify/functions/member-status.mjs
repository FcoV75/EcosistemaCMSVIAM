...
function tokenPremium(normalized, producto) {
  ...
  return createAccessToken(
    { sub: normalized, tier: 'premium', product: producto },
    secret,
  );
}
  ...
  try {
    ...
    if (productoRequerido === 'sincronia_nexus' && esCodigoPropietarioNexus(normalized)) {
      return respuestaMembresia(
        estadoMembresia(normalized, { producto: 'sincronia_nexus', plan: 'propietario' }),
        normalized,
        'sincronia_nexus',
      );
    }
    ...
    if (productoRequerido === 'video_diamante_premium' && esCodigoPropietarioVideoDiamante(normalized)) {
      return respuestaMembresia(
        estadoMembresia(normalized, { producto: 'video_diamante_premium', plan: 'propietario' }),
        normalized,
